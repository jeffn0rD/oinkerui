...
1. **ERROR**: Task 0.3.5 still in master_todo.yaml
   - File: `master_todo.yaml`
   - Action: Remove task 0.3.5 from master_todo.yaml

2. **ERROR**: Task 0.3.5 not in tasks_completed.yaml
   - File: `log/tasks_completed.yaml`
   - Action: Add task 0.3.5 to tasks_completed.yaml
...
expected <block end>, but found '<scalar>'
  in "<unicode string>", line 75, column 24:
        fastify: "^4.29.1" (spec: ^4.26.0) ✓
                           ^
   - File: `log/task_0.3.5_summary.yaml`
...
- Remove task 0.3.5 from master_todo.yaml
- Add task 0.3.5 to tasks_completed.yaml
...
Task 0.3.5 cleanup found 5 issue(s) that need to be resolved.
...
```bash
python3 tools/doc_query.py --query "0.3.5" --mode text --pretty
