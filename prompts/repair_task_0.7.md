...
1. **ERROR**: Task 0.7 still in master_todo.yaml
   - File: `master_todo.yaml`
   - Action: Remove task 0.7 from master_todo.yaml

2. **ERROR**: Task 0.7 not in tasks_completed.yaml
   - File: `log/tasks_completed.yaml`
   - Action: Add task 0.7 to tasks_completed.yaml
...
- Remove task 0.7 from master_todo.yaml
- Add task 0.7 to tasks_completed.yaml
...
Task 0.7 cleanup found 3 issue(s) that need to be resolved.
...
```bash
python3 tools/doc_query.py --query "0.7" --mode text --pretty
