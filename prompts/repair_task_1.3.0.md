...
1. **ERROR**: Task 1.3.0 still in master_todo.yaml
   - File: `master_todo.yaml`
   - Action: Remove task 1.3.0 from master_todo.yaml

2. **ERROR**: Task 1.3.0 not in tasks_completed.yaml
   - File: `log/tasks_completed.yaml`
   - Action: Add task 1.3.0 to tasks_completed.yaml


## Required Actions

- Remove task 1.3.0 from master_todo.yaml
- Add task 1.3.0 to tasks_completed.yaml
...
Task 1.3.0 cleanup found 2 issue(s) that need to be resolved.
...
```bash
python3 tools/doc_query.py --query "1.3.0" --mode text --pretty
