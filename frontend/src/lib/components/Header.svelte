<script>
  ...
  let { onSettings = () => {}, onProfile = () => {} } = $props();
...
<header class="flex items-center justify-between px-4 py-2 bg-surface border-b border-border">
  <div class="text-sm font-medium text-foreground">OinkerUI</div>
  
  <div class="flex items-center gap-3">
    <!-- Connection status -->
    <div class="flex items-center gap-1.5 text-xs">
      <div class="w-2 h-2 rounded-full {connected ? 'bg-green-500' : 'bg-red-500'}"></div>
      <span class="text-muted">{connected ? 'Connected' : 'Disconnected'}</span>
    </div>
